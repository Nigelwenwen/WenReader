...
import com.nigel.wenreader.utils.IOUtils;
...
public abstract class PageLoader implements Serializable {
    ...
    private static final int DEFAULT_PERCENT_SIZE = 8;
    private static final int EXTRA_TITLE_SIZE = 4;

    private Context mContext;
    //用户设置类
    PreSettingsManager mSettingsManager;
    //仓库
    ...
    //书本对象
    protected CollectBookEntity mCollectBook;
    ...
    //绘制电池百分比的画笔
    private TextPaint mPercetnPaint;
    ...
    // 存储阅读记录类
    private BookRecordEntity mBookRecord;
    // 被遮盖的页，或者认为被取消显示的页
    private TxtPage mCancelPage;



    public PageLoader(PageView pageView,CollectBookEntity collectBookEntity) {
        mCollectBook = collectBookEntity;
        mPageView = pageView;
    ...
    private void initData() {
        ...
        mPageMode=mSettingsManager.getPageMode();
        Log.d(TAG, "initData: pageMode "+mPageMode);
        mPageStyle=mSettingsManager.getPageStyle();
        mMarginHeight=ScreenUtils.dpToPx(DEFAULT_MARGIN_HEIGHT);
        mMarginWidth=ScreenUtils.dpToPx(DEFAULT_MARGIN_WIDTH);
        //设置文字相关参数
        setUpTextParams(mSettingsManager.getTextSize());
    }
    ...
    private void setUpTextParams(int textSize) {
        //设置正文和标题的字体大小
        mTextSize=textSize;
        mTitleSize=mTextSize+ScreenUtils.dpToPx(EXTRA_TITLE_SIZE);
        // 行间距(大小为字体的一半)
    ...
    private void initPaint() {
        ...
        //绘制电池百分比的画笔
        mPercetnPaint=new TextPaint();
        mPercetnPaint.setTextAlign(Paint.Align.CENTER);
        mPercetnPaint.setTextSize(ScreenUtils.spToPx(DEFAULT_PERCENT_SIZE));
        mPercetnPaint.setAntiAlias(true);
        mPercetnPaint.setSubpixelText(true);
        mPercetnPaint.setStrokeWidth(2);
        ...
        // 初始化页面样式
        setNightMode(mSettingsManager.isNightMode());
    }
    ...
    private void initPageView(){
        //配置参数
//        mPageView.setPageMode(3);
        mPageView.setPageMode(mPageMode);
//        mPageView.setBgColor(ContextCompat.getColor(App.getContext(), R.color.light_red));
    ...
    public void setPageMode(int pageMode) {
        ...
        mPageView.setPageMode(mPageMode);
        mSettingsManager.setPageMode(mPageMode);
    ...
    public void setNightMode(boolean nightMode) {
        mSettingsManager.setNightMode(nightMode);
        isNightMode = nightMode;

        if (isNightMode) {
            Log.d(TAG, "setNightMode: night");
            mBatteryPaint.setColor(Color.WHITE);
            setPageStyle(PageStyle.NIGHT);
        } else {
            Log.d(TAG, "setNightMode: sun");
    ...
    public void setPageStyle(PageStyle pageStyle) {
        if(pageStyle!=PageStyle.NIGHT){
            ...
            mSettingsManager.setPageStyle(mPageStyle);
            Log.d(TAG, "colorChange: "+mPageStyle.getBgColor());
        }
        if(isNightMode&&pageStyle!=PageStyle.NIGHT){
            Log.d(TAG, "setPageStyle: return");
            return;
        }
        ...
        mPercetnPaint.setColor(mTextColor);
        mBatteryPaint.setColor(mTextColor);

        mBgPaint.setColor(mBgColor);

        mPageView.drawCurPage(false);
//        this.drawPage(mPageView.getNextBitmap(),false);
    }
    ...
    public void setTextSize(int textSize) {
        ...
        mTitlePaint.setTextSize(mTitleSize);
        // 存储文字大小
        mSettingsManager.setTextSize(mTextSize);
        // 取消缓存
        mPrePageList = null;
        mNextPageList = null;
    ...
    public void openChapter() {
        Log.d(TAG, "openChapter: open");
        isFirstOpen = false;

        if (!mPageView.isPrepare()) {
            Log.d(TAG, "openChapter: not prepare");
            return;
        }
    ...
    private void drawBackground(Bitmap bgBitmap, boolean isUpdate) {
        ...
        //百分比的制作
        float centerY=outFrame.centerY();
        float centerX=outFrame.centerX();
        Paint.FontMetrics metrics=mPercetnPaint.getFontMetrics();

        float baseline=centerY+(metrics.bottom-metrics.top)/2-metrics.bottom;
        canvas.drawText(""+mBatteryLevel,centerX,baseline,mPercetnPaint);
    ...
    private void prepareBook() {
        mBookRecord =mRepository.getBookRecord(mCollectBook.get_id());

        if (mBookRecord == null) {
            mBookRecord = new BookRecordEntity();
        }
    ...
    public void saveRecord() {
        ...
        mBookRecord.setBookId(mCollectBook.get_id());
        ...
        //存储到数据库
        mRepository.saveBookRecord(mBookRecord);
    }
    ...
    private void preLoadNextChapter() {
        ...
        Single.create(new SingleOnSubscribe<List<TxtPage>>() {
            @Override
            public void subscribe(SingleEmitter<List<TxtPage>> emitter) throws Exception {
                emitter.onSuccess(loadPageList(nextChapter));
            }
                ...
                .subscribe(new SingleObserver<List<TxtPage>>() {
                    ...
                    @Override
                    public void onSuccess(List<TxtPage> txtPages) {
    ...
    void pageCancel(){
        if(mCurPage.position==0&&mCurChapterPos>mLastChapterPos){   //取消加载下一章
            if (mPrePageList != null) {
                cancelNextChapter();
            } else {
                if (parsePrevChapter()) {
    ...
    private List<TxtPage> loadPageList(int chapterPos) throws Exception {
        ...
        List<TxtPage> pages = loadPages(chapter, reader);
